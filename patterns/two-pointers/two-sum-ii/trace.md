...
`left` and `right` below are 0-based pointer indexes. The final `answer` is 1-based, as required by the classic Two Sum II problem.
...
| 3 | Compare `numbers[1] + numbers[3]`. | 1 | 3 | `[2, 6]` | 8 | Sum equals target, so the answer is found. | Pointer indexes `[1, 3]` convert to the 1-based answer `[2, 4]`. |
